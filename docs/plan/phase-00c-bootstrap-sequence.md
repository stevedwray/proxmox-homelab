...
subsequent container deployments source images from Harbor at `10.57.3.10`. No container
...
./with-secrets bash -c \
  'curl -sk -u "admin:${HARBOR_ADMIN_PASSWORD}" http://10.57.3.10/api/v2.0/scanners | jq ".[] | {name, is_default, health}"'
...
```bash
./with-secrets bash -c \
  'echo "${HARBOR_ROBOT_PASSWORD}" | docker login 10.57.3.10 -u "${HARBOR_ROBOT_USER}" --password-stdin'
```
...
docker pull 10.57.3.10/dockerhub/goharbor/harbor-core:v2.11.0
docker pull 10.57.3.10/dockerhub/goharbor/harbor-db:v2.11.0
docker pull 10.57.3.10/dockerhub/goharbor/harbor-jobservice:v2.11.0
docker pull 10.57.3.10/dockerhub/goharbor/harbor-portal:v2.11.0
docker pull 10.57.3.10/dockerhub/goharbor/harbor-registryctl:v2.11.0
docker pull 10.57.3.10/dockerhub/goharbor/registry-photon:v2.11.0
docker pull 10.57.3.10/dockerhub/goharbor/nginx-photon:v2.11.0
docker pull 10.57.3.10/dockerhub/goharbor/redis-photon:v2.11.0
docker pull 10.57.3.10/dockerhub/goharbor/trivy-adapter-photon:v2.11.0
```
...
All stage-2 Ansible playbooks reference images at `10.57.3.10/...` via robot account
...
| IP | `10.57.1.20/24` |
| Gateway | `10.57.1.1` |
| Image source | Stage 1 Harbor (`10.57.3.10`) |
...
**Record in NetBox:** Update the VMID 120 record to reflect Stage 2 status.
...
**Record in NetBox:** Update the VMID 141 record to reflect Stage 2 status.
...
**Record in NetBox:** Update the VMID 121 record to reflect Stage 2 status and note that
...
- [ ] Stage 1 Portainer is running at `10.57.1.20` (VMID 120) and API responds
- [ ] Stage 1 Harbor is running at `10.57.3.10` (VMID 121) and returns `"Pong"`
...
- [ ] Stage 2 Portainer is running at `10.57.1.20` (VMID 120, replacing Stage 1)
- [ ] Stage 2 CI runner is running at `10.57.0.63` (VMID 141, replacing Stage 1)
- [ ] Stage 2 Harbor is running at `10.57.3.10` (VMID 121, replacing Stage 1)
- [ ] Stage 1 Portainer LXC is absent (`qm status 120` returns not found or stopped)
- [ ] Stage 1 Harbor LXC is absent (`qm status 121` returns not found or stopped)
- [ ] Stage 1 CI runner LXC is absent (`qm status 141` returns not found or stopped)
...
- [ ] NetBox records updated for VMIDs 120, 121, and 141
