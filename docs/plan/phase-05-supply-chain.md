  ...
  trivy-image-scan:
    ...
    steps:
      ...
      - name: Run Trivy image scan
        ...
        with:
          scan-type: image
          image-ref: "192.168.40.10/<project>/<image>:<tag>"
          format: sarif
  ...
  generate-sbom:
    ...
    steps:
      - name: Generate SBOM (SPDX)
        run: |
          syft 192.168.40.10/<project>/<image>:<tag> \
      ...
      - name: Sign image
        ...
        run: |
          ...
          cosign sign --key /tmp/cosign.key \
            192.168.40.10/<project>/<image>@<digest>
          rm /tmp/cosign.key
```
      ...
      - name: Check compose files use Harbor proxy
        run: |
          ...
          if grep -r "^\s*image:.*\(docker\.io\|ghcr\.io\|quay\.io\|registry\.k8s\.io\)" \
            ...
            echo "ERROR: Direct upstream registry references found. Use 192.168.40.10/... instead."
            exit 1
          fi
...
- [ ] All compose files for Phase 06+ stacks reference `192.168.40.10/<project>/` only
