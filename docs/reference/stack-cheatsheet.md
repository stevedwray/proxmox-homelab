...
| `dns-stack` | ~~20013~~ | ~~`mgmt_seg`~~ | ~~192.168.20.13~~ | **REMOVED from `pve`, 2026-09-08** — CoreDNS, superseded by `technitium-stack` since the 2026-07-05 cutover. Decommissioned only after confirming zero remaining dependents (the Docker-daemon-level DNS hardcoding that broke Harbor's pulls fleet-wide on 2026-08-16 — see `reference_dns_stack_docker_daemon_dependency` — was fixed across all 8 affected stacks first). `pve-test-vm` still has its own separate `dns-stack` instance; this row only ever described the `pve` one. |
