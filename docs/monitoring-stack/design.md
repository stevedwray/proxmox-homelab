...
All other `LAB_IP_*` variables are in `.env`. No secrets required for VictoriaLogs or VictoriaMetrics (both are mgmt_seg-internal, no auth).
...
`task/monitoring-victorialogs` — cut from `baseline/teardown-validated` @ 6c38905
