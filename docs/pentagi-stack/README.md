...
- **Traefik yes, Authentik native-OIDC no.** PentAGI's own login only
  ...
  behind that gate, not the primary control. **Not yet confirmed**: whether
  `reconcile-authentik-edge.py` already automates the Authentik-side Proxy
  Provider for `forwardAuth` routes, since every existing example
  (`discover-authentik-edge.py`'s tables) uses native `oidc` mode instead —
  needs a direct check before assuming it's zero-touch.
- **Model policy**: `PRIMARY_MODEL` is Llama-3.3-70B-Instruct, not the Qwen
  models used in earlier hands-on PentAGI testing — `decisions.md`
  Decision 12 is an evidence-based ban on Qwen for reliable structured tool
  calls. That evidence is from VSCode/Copilot/Continue, though, not PentAGI's
  own provider calibration — Phase 2's tool-call preflight is the first
  direct confirmation either way, not a formality.
