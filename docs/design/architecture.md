...
|---|---|---|
| SEC-01 | CI runner (`build_seg`) egress must be restricted at MikroTik — allow only Harbor (`192.168.40.10`), apt-cacher (`192.168.40.11`), GitHub API/Actions, and package registries. Block all other outbound. | Compromised dependency → runner pivot to infra_seg via unrestricted egress |
