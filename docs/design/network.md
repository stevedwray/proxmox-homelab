...
| `dns_server` not set in all stacks | — | `proxy-stack` (edge_seg) and `harbor-stack` (infra_seg) do not set `dns_server` in `stack.yaml`. DNS may fall back to template defaults on rebuild. |
