...
1. **PR `feat/netbox-populate-multi-source-inventory` → `dev/pve-test`** — this is
   application stack work; no teardown gate required. Validation has been done live
   on pve-test with both nodes populating correctly.
