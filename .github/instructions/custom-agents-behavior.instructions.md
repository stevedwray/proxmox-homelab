...
- executors should fully replace `output_report` for the current session rather than appending to an older copy, and should confirm the final report starts with the current `## Session <id>` heading and does not contain a stale duplicate block for that same session.
